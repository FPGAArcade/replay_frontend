...
pub enum TextureFormat {
    ...
    R8Linear = 0,
    /// 3 byte R,G,B format (sRGB) The backend needs to either convert this to linear format or use a GPU compatible format that does this in runtime
    Rgb8Srgb = 1,
    /// 3 byte R,G,B format (LINEAR)
    Rgb8Linear = 2,
    /// 4 byte R,G,B,A format (sRGB) The backend needs to either convert this to linear format or use a GPU compatible format that does this in runtime
    Rgba8Srgb = 3,
