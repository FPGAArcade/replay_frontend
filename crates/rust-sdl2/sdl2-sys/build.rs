...
fn compile_sdl2(sdl2_build_path: &Path, target_os: &str) -> PathBuf {
    ...
    // Use the configure script specifically for aarch64 Linux targets
    if target == "aarch64-unknown-linux-gnu" {
        link_with_static_lib(sdl2_build_path)
    } else {
        compile_sdl2_cmake(sdl2_build_path, target_os)
    }
}

#[cfg(feature = "bundled")]
fn link_with_static_lib(sdl2_build_path: &Path) -> PathBuf {
    // Set the library path for linking to the installed static library
    println!("cargo:rustc-link-search=native={}/../lib/aarch64-unknown-linux-gnu", sdl2_build_path.display());
    // SDL2 may need these system libraries when statically linked
...
#[cfg(feature = "bundled")]
fn compile_sdl2_cmake(sdl2_build_path: &Path, target_os: &str) -> PathBuf {
